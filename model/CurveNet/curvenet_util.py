...
class LPFA(nn.Module):
    ...
    def group_feature(self, x, xyz, idx):
        ...
        # (batch_size, num_points, self.k, 3) -> (batch_size, num_points, self.k, -1)
...
class LPFA_bo(nn.Module):
    def __init__(self, in_channel, out_channel, k, mlp_num=2, initial=False):
        super(LPFA_bo, self).__init__()
        self.k = k
        # self.device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        self.initial = initial

        if not initial:
            # For Normal (9, in_channel, kernel_size=1, bias=False)
            # For Centerline (12, in_channel, kernel_size=1, bias=False)
            # For Centerline+Curvature (15, in_channel, kernel_size=1, bias=False)
            self.xyz2feature = nn.Sequential(
                        nn.Conv2d(12, in_channel, kernel_size=1, bias=False),
                        nn.BatchNorm2d(in_channel))
...
class LPFA_ob(nn.Module):
    def __init__(self, in_channel, out_channel, k, mlp_num=2, initial=False):
        super(LPFA_ob, self).__init__()
        self.k = k
        # self.device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        self.initial = initial

        if not initial:
            # (9, in_channel, kernel_size=1, bias=False) -> (12, in_channel, kernel_size=1, bias=False)
            self.xyz2feature = nn.Sequential(
                        nn.Conv2d(12, in_channel, kernel_size=1, bias=False),
                        nn.BatchNorm2d(in_channel))
...
class CIC_bo(nn.Module):
    def __init__(self, npoint, radius, k, in_channels, output_channels, bottleneck_ratio=2, mlp_num=2,
                 curve_config=None):
        super(CIC_bo, self).__init__()
        self.in_channels = in_channels
        self.output_channels = output_channels
        self.bottleneck_ratio = bottleneck_ratio
        self.radius = radius
        ...
        self.lpfa = LPFA_bo(planes, planes, k, mlp_num=mlp_num, initial=False)
...
class CIC_ob(nn.Module):
    def __init__(self, npoint, radius, k, in_channels, output_channels, bottleneck_ratio=2, mlp_num=2,
                 curve_config=None):
        super(CIC_ob, self).__init__()
        self.in_channels = in_channels
        self.output_channels = output_channels
        self.bottleneck_ratio = bottleneck_ratio
        self.radius = radius
        ...
        self.lpfa = LPFA_ob(planes, planes, k, mlp_num=mlp_num, initial=False)
